...
//    private static String YOUR_URL ="http://218.244.151.190/demo/charge";
    private static String YOUR_URL ="http://120.26.225.230:819/index.php/Api/Charge/recharge";
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        final ProgressDialog pd = ProgressDialog.show(this, "", "获取数据中...", false, true);
        pd.show();
        handler=new Handler();
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.upmpButton) {
            new PaymentTask().execute(new PaymentRequest(CHANNEL_UPACP,"9981925", amount));
        } else if (view.getId() == R.id.alipayButton) {
            new PaymentTask().execute(new PaymentRequest(CHANNEL_ALIPAY,"9981925", amount));
        } else if (view.getId() == R.id.wechatButton) {
            new PaymentTask().execute(new PaymentRequest(CHANNEL_WECHAT,"9981925", amount));
        } else if (view.getId() == R.id.bfbButton) {
        	new PaymentTask().execute(new PaymentRequest(CHANNEL_BFB,"9981925", amount));
        } else if(view.getId() == R.id.jdpayButton){
        	new PaymentTask().execute(new PaymentRequest(CHANNEL_JDPAY_WAP,"9981925", amount));
        } else if(view.getId() == R.id.qpayButton){
//            new PaymentTask().execute(new PaymentRequest(CHANNEL_QPAY,"9981925", amount));
            Toast.makeText(ClientSDKActivity.this,"点击了qq钱包支付",Toast.LENGTH_SHORT).show();
        }
    }
...
//            Pingpp.createPayment(ClientSDKActivity.this, data, "qwalletXXXXXXX");
    ...
    class PaymentRequest {
        ...
        public PaymentRequest(String channel,String uid, int amount) {
